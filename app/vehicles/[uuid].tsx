...
import { deleteRegisteredVehicle, getRegisteredVehicleDetail } from '@/services/api';
...
export default function VehicleDetailScreen() {
    ...
    return (
        <ScrollView style={[styles.container, { backgroundColor: colors.background }]}>
            <Stack.Screen options={{ title: vehicle.plate_number || 'Vehicle Details', headerBackTitle: 'Vehicles' }} />

            <View style={styles.header}>
                <View style={[styles.avatarContainer, { backgroundColor: colors.card, shadowColor: colors.shadow }]}>
                    <Ionicons name="car-sport" size={60} color={colors.primary} />
                </View>
                <Text style={[styles.title, { color: colors.text }]}>
                    {vehicle.vehicle_name || `${vehicle.vehicle_details?.brand || ''} ${vehicle.vehicle_details?.model || ''}`}
                </Text>
            ...
            <View style={[styles.card, { backgroundColor: colors.card }]}>
                ...
                <DetailItem
                    ...
                    label="Type"
                    value={vehicle.vehicle_type_display || vehicle.vehicle_type || 'Individual'}
                />
                {vehicle.vehicle_details?.battery_size && (
                    <DetailItem
                        icon="battery-charging-outline"
                        label="Battery Size"
                        value={`${vehicle.vehicle_details.battery_size} kWh`}
                    />
                )}
            </View>
