...
import { getRegisteredVehicleDetail, getVehicleModels, updateRegisteredVehicle } from '@/services/api';
...
export default function UpdateVehicleScreen() {
    ...
    const loadData = async () => {
        try {
            ...
            if (vehicleData.vehicle) {
                const found = modelList.find((m: any) => m.uuid === vehicleData.vehicle || m.id === vehicleData.vehicle);
                if (found) setSelectedModel(found);
            } else if (vehicleData.vehicle_details?.uuid) {
                const found = modelList.find((m: any) => m.uuid === vehicleData.vehicle_details.uuid);
                if (found) setSelectedModel(found);
            } else if (vehicleData.vehicle_details?.id) {
                const found = modelList.find((m: any) => m.id === vehicleData.vehicle_details.id);
                if (found) setSelectedModel(found);
            }
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            const msg = error.response?.data?.message || "Could not update vehicle. Please try again.";
    ...
    return (
        <View style={[styles.container, { backgroundColor: colors.background }]}>
            <Stack.Screen options={{ title: 'Update Vehicle', headerBackTitle: 'Cancel' }} />
