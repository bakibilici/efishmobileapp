...
function getVehicleDisplayName(item: any): string {
  // New backend format:
  // item.vehicle.model.brand.name, item.vehicle.model.name, item.vehicle.name
  const brand =
  ...
  // Avoid duplicate brand/model when variant already contains them
  return parts.join(" ");
}
...
export default function VehiclesScreen() {
  ...
  const fetchVehicles = async () => {
    try {
      ...
      // New backend structure: { success, message_key, data: { count, next, previous, results: [...] } }
      let list: any[] = [];
      ...
      const withNames = list.map((item) => ({
        ...item,
        vehicle_name: item.vehicle_name || getVehicleDisplayName(item),
      }));

      setVehicles(withNames);
    } catch (error) {
      ...
      <View style={styles.cardContent}>
        ...
        <View style={styles.cardInfo}>
          ...
          <Text style={[styles.modelName, { color: colors.textSecondary }]}>
            {item.vehicle_name || getVehicleDisplayName(item)}
          </Text>
        </View>
        <Ionicons
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <Stack.Screen
        options={{ title: "My Vehicles", headerBackTitle: "Profile" }}
      />
