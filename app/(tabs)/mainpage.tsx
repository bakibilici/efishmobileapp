...
const MapPinMarker = React.memo(
  ({
    ...
    const availableCount = station.socket_stats
      ? Object.values(station.socket_stats).reduce(
          (acc: number, curr: any) => acc + (curr.available || 0),
          0,
        )
      : 0;
...
export default function MapScreen() {
  ...
  const connectMeterValuesSocket = useCallback(
      ...
      try {
        ...
        ws.onmessage = (event) => {
          ...
          try {
            ...
            if (data.type === "socket_status") {
              ...
              console.log(
                "🔌 SOCKET CHANGE RECEIVED:",
                JSON.stringify(statusData, null, 2),
              );
              if (statusData) {
                // 1. Update StationDetails (if open)
                setStationDetails((currentDetails: any) => {
                  if (!currentDetails || !currentDetails.charge_points)
                    return currentDetails;

                  let hasChange = false;
                  const updatedPoints = currentDetails.charge_points.map(
                    (cp: any) => {
                  ...
                  return hasChange
                    ? { ...currentDetails, charge_points: updatedPoints }
                    : currentDetails;
                });
                ...
                if (
                  (upperStatus === "CHARGING" ||
                    upperStatus === "PREPARING" ||
                    upperStatus === "INITIATED") &&
                  !chargingIsActiveRef.current
                ) {
                  if (!reconnectTimeoutRef.current) {
                    console.log(
                      "Possible external start detected! Reconnecting meter-values socket...",
                    );
            ...
            if (
              data.type === "socket_status" ||
              data.type === "meter_values" ||
              data.type === "charge_session"
            ) {
              rawData = data.data || {};
            }
            ...
            if (
              data.type === "meter_values" &&
              !chargingIsActiveRef.current &&
              (rawData.power > 0 || rawData.batteryLevel > 0)
            ) {
              console.log(
                "Surprise meter_values received! Reconnecting meter-values socket to fetch true started_at.",
              );
              if (!reconnectTimeoutRef.current) {
                reconnectTimeoutRef.current = setTimeout(() => {
                  reconnectTimeoutRef.current = null;
            ...
            if (
              ...
              if (activeChargeSessionUuidRef.current || charging.isActive) {
                ...
                if (
                  upperStatus === "FINISHING" ||
                  upperStatus === "SUSPENDEDEV" ||
                  upperStatus === "SUSPENDEDEVSE" ||
                  upperStatus === "FAULTED"
                ) {
            ...
            if (
              ...
              payload = {
                ...
                cost:
                  d.price != null && d.total_energy != null
                    ? d.price * d.total_energy
                    : undefined,
        ...
        ws.onclose = (event) => {
          ...
          if (event.reason && event.reason.includes("403")) {
            console.warn(
              "WebSocket Auth Failed (403). Stopping reconnect loop.",
            );
            return;
          }
          ...
          if (user && !reconnectTimeoutRef.current) {
            ...
            console.log(
              `WebSocket Reconnecting in ${delay}ms (Attempt ${attempt})`,
            );
  ...
  const handleStopCharging = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(
        "Error",
        e.response?.data?.message || "Failed to stop charging session.",
      );
    } finally {
    ...
    try {
      ...
      const initialSoc =
        session?.battery_level != null
          ? Number(session.battery_level)
          : undefined;
      const initialEnergy =
        session?.total_energy != null
          ? parseFloat(String(session.total_energy))
          : undefined;
      ...
      if (initialSoc != null || initialEnergy != null) {
        charging.updateFromMeterValues({
          ...
          power_kw:
            session?.avg_charge_power != null
              ? parseFloat(String(session.avg_charge_power))
              : undefined,
        } as any);
      }
      ...
      if (e.response?.data?.message_key === "socket.not_plugged") {
        ...
        Alert.alert(
          "Error",
          e.response?.data?.message || "Failed to start charging session.",
        );
      }
    }
  };
  ...
  const { headerMode, reportInteraction, forceExpand } =
    useHeaderAutoCollapse(activityState);
  const isHeaderExpanded = headerMode === "FULL";

  const headerExpandAnim = useRef(
  ...
  useEffect(() => {
    ...
    const notPluggedSubscription = DeviceEventEmitter.addListener(
      "evt_QR_NOT_PLUGGED",
      () => {
  ...
  const renderBackdrop = useCallback((props: any) => null, []);
        ...
        <View style={styles.container}>
                  ...
                  <>
                          ...
                          >
                            ...
                            <View style={{ flexDirection: "row" }}>
                              ...
                              >
                                <Ionicons
                                  name={
                                    showAddress ? "chevron-up" : "chevron-down"
                                  }
                                  size={20}
                                  color={colors.textSecondary}
                                />
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          {selectedStation.type} {selectedStation.powerKw} kW
                        </Text>
                      </View>
                      ...
                      >
                        ...
                        >
                          {selectedStation.is_public !== undefined
                            ? selectedStation.is_public
                              ? "Public"
                              : "Private"
                            : "Public"}
                    ...
                    {isFetchingDetails && !stationDetails ? (
                      ...
                      >
                        ...
                        />
                        <Text
                          style={{ marginTop: 10, color: colors.textTertiary }}
                        >
                          Loading details...
                        </Text>
                      </View>
                        ...
                        {stationDetails.charge_points?.map((cp: any) => (
                          <View key={cp.id} style={{ marginBottom: 12 }}>
                            ...
                            {cp.sockets?.map((socket: any) => {
                              ...
                              if (status === "available")
                                statusColor = "#4BACE4"; // Blue
                              else if (status === "preparing")
                                statusColor = "#0093C9"; // Blue
                              else if (status === "charging")
                                statusColor = "#FFCC00"; // Yellow
                              else if (status === "unavailable")
                                statusColor = "#B366FF"; // Purple
                              else if (status === "faulted")
                                statusColor = "#FF3B30"; // Red
                              else if (status === "reserved")
                                statusColor = colors.border; // Grayish
                                  ...
                                  >
                                    ...
                                    {status === "available" ? (
                                      ...
                                      <View
                                        style={{
                                          backgroundColor:
                                            "rgba(255, 204, 0, 0.15)", // Light Yellow bg
                                      ...
                                      <View
                                        style={{
                                          backgroundColor:
                                            "rgba(179, 102, 255, 0.15)", // Light Purple bg
                                          paddingHorizontal: 12,
                                          paddingVertical: 6,
                                          borderRadius: 88,
                                          borderWidth: 1,
                                      ...
                                      <View
                                        style={{
                                          backgroundColor:
                                            "rgba(255, 59, 48, 0.15)", // Light Red bg
                                          paddingHorizontal: 12,
                                          paddingVertical: 6,
                                          borderRadius: 88,
                                          borderWidth: 1,
          ...
          {showSuccessToast && (
            ...
            >
              ...
              <Text
                style={{ color: "#0b2319", fontWeight: "700", fontSize: 13 }}
              >
              ...
              <BottomSheetFlatList
                ...
                ListEmptyComponent={() => (
                  ...
                  >
                    ...
                    >
                      There are no stations matching the selected type in this
                      area.
                    </Text>
                  </View>
                )}
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          {Array.from({
                            length:
                              typeLightningCount[item.type as StationType] || 1,
                          }).map((_, i) => (
                            <View
                              key={i}
                              style={{ marginLeft: i > 0 ? -12 : 0, zIndex: i }}
                            >
                              <Ionicons
                                name="flash"
                                size={20}
        ...
        >
          {lightningCount ? (
            <View
              style={{
                flexDirection: "row",
                marginRight: label ? (compact ? 4 : 6) : 0,
              }}
            >
          ...
          {label && label.length > 0 ? (
            <Animated.Text
              style={[
                styles.chipText,
                { color: textColor, fontSize: compact ? 15 : 15 },
              ]}
            >
              {label}
            </Animated.Text>
